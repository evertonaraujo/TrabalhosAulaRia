...
Ext.define('EIA.controller.Fluxo', {
    ...
    delete: function() {
        ...
        if(records.length === 0){
			...
			Ext.Ajax.request ({
				scope	: this,
				url		: 'php/fluxos.php?acao=delete', //arquivo que contém o método a utilizar
				params	: {
				...
				success: function(r){ // se a exclusão foi executada com sucesso
					...
					if(obj.success){						
						...
						//this.getFluxoList().store.load(); // atualiza informações do grid												 
					}
					else{
        ...
        if (form.isValid()) {
			...
			{												
				...
				{						
					Ext.Ajax.request ({
						scope	: this,
						url		: 'php/fluxos.php?acao=update', //arquivo que contém o método a utilizar
						params	: {
						'idFluxo'					: idFluxo,
